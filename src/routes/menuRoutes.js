...
// Unified get menu items with optional filtering by category
exports.getMenuItems = async (req, res) => {
  try {
    ...
    let query = `SELECT ${selectFields.join(', ')} FROM menu_items WHERE 1=1`;
    const params = [];

    if (availableColumns.includes('is_available')) {
      query += ' AND is_available = TRUE';
    }

    if (req.query.category && availableColumns.includes('category')) {
      query += ' AND category = ?';
    ...
    res.status(500).json({ success: false, message: 'Server error when fetching menu items' });
  }
};

// Get menu items by category (strict "hot"/"cold")
